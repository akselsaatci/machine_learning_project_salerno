...
parser.add_argument("--env", default="CartPoleContinuousBulletEnv-v0",
                    help="the environment on which the agent should be trained "
                         "(Default: CartPoleContinuousBulletEnv-v0)")
parser.add_argument("--render_train", default=True, type=bool,
...
if args.env.startswith("list"):
    if args.env == "list" or args.env == "list-all":
        from gym import envs
        all_envs = envs.registry.all()
        env_ids = [env_spec.id for env_spec in all_envs]
    elif args.env == "list-pybullet":
        env_ids = pybullet_envs.getList()
    else:
        from gym import envs
        all_envs = envs.registry.all()
        id_path = args.env[5:]
        env_ids = [env_spec.id for env_spec in all_envs if id_path in env_spec.id]

    num_envs = len(env_ids)

    print(num_envs, 'environments available:')
    for i, env_id in enumerate(sorted(env_ids)):
        print(i+1, env_id)
    exit(0)

elif args.env.startswith("info-"):
    from gym import envs
    from pprint import pprint
    all_envs = envs.registry.all()
    num_envs = len(all_envs)
    for env_spec in all_envs:
        if env_spec.id == args.env[5:]:
            print('Environment specs for', args.env[5:])
            pprint(vars(env_spec))
    exit(0)
...
if __name__ == "__main__":
    ...
    # Create the env
    kwargs = dict()
    if args.env == 'CartPoleContinuousBulletEnv-v0':
        kwargs['renders'] = True
    env = RobotTableEnv()
